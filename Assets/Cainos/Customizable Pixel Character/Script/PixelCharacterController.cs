...
namespace Cainos.CustomizablePixelCharacter
{
    public class PixelCharacterController : MonoBehaviour
    {
        ...
        public void Dash()
        {
            ...
            if (isDrawingBow || isPointingAtTarget) return;
        ...
        {
            if (isDead || ledgeClimbEnabled == false || isClimbingLadder || isExitingLadder || isDrawingBow || isCrawling )
            {
                isClimbingLedge = false;
                return;
            }
        ...
        {
            if ( isExitingLadder == false && isExitingLadder == false && isClimbingLedge == false ) return;

            if ( Mathf.Abs(rootMotionReceiver.rootMotionVel.magnitude) > MOVE_THRESHOLD)
            {
                velocity = rootMotionReceiver.rootMotionVel;
            }
        }
